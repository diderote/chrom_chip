...
def clean_encode_folder(exp):
    ...
    for sample in exp.IPs.Condition.unique().tolist():
        ...
        bw = globber(f'{crom_dir}call-macs2/shard-0/execution/*fc.signal.bigwig')
        sample_peak = globber(f'{crom_dir}call-macs2/shard-0/execution/*bfilt.narrowPeak.gz')
        nodup_bam = globber(f'{crom_dir}call-filter/shard-0/execution/*.nodup.bam')
        nodup_bai = globber(f'{crom_dir}call-filter/shard-0/execution/*.nodup.bam.bai')
        nodup_input_bam = globber(f'{crom_dir}call-filter_ctl/shard-0/execution/*nodup.bam')
        nodup_input_bai = globber(f'{crom_dir}call-filter_ctl/shard-0/execution/*nodup.bam.bai')
        bam = globber(f'{crom_dir}call-bwa/shard-0/execution/*.bam')
        bai = globber(f'{crom_dir}call-bwa/shard-0/execution/*.bam.bai')
        input_bam = globber(f'{crom_dir}call-bwa_ctl/shard-0/execution/*.bam')
        input_bai = globber(f'{crom_dir}call-bwa_ctl/shard-0/execution/*.bam.bai')
        optimal_peak = globber(f'{crom_dir}call-reproducibility_overlap/execution/optimal_peak.narrowPeak.gz')
        conservative_peak = globber(f'{crom_dir}call-reproducibility_overlap/execution/conservative_peak.narrowPeak.gz')
        qc_report = globber(f'{crom_dir}call-qc_report/execution/qc.html')
        json = globber(f'{encode_dir}{sample}/*.json')

        ### TO DO: UPDATE FOR REPLICATES ###

        rename(bw, f'{exp.scratch}bws/{sample}.fc.signal.bigwig')
        rename(sample_peak, f'{exp.scratch}sample_peaks/{sample}.500K.bfilt.narrowPeak.gz')
        rename(nodup_bam, f'{exp.scratch}nodup_bam/{sample}.nodup.bam')
        rename(nodup_bai, f'{exp.scratch}nodup_bam/{sample}.nodup.bam.bai')
        rename(nodup_input_bam, f'{exp.scratch}nodup_bam/{sample}_background.nodup.bam')
        rename(nodup_input_bai, f'{exp.scratch}nodup_bam/{sample}_background.nodup.bam.bai')
        rename(bam, f'{exp.scratch}bams/{sample}.bam')
        rename(bai, f'{exp.scratch}bams/{sample}.bam.bai')
        rename(input_bam, f'{exp.scratch}bams/{sample}_background.bam')
        rename(input_bai, f'{exp.scratch}bams/{sample}_background.bam.bai')
        rename(optimal_peak, f'{exp.scratch}rep_peaks/{sample}.optimal_peak.narrowPeak.gz')
        rename(conservative_peak, f'{exp.scratch}rep_peaks/{sample}.conservative_peak.narrowPeak.gz')
